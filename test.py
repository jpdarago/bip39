...
def encode_bytes(entropy: bytes) -> str:
    ...
    print(num_bits_checksum, checksum)
    ...
    print(hex(entropy_and_checksum))

    # Convert each 11 bit chunk into a word.
    remaining_data = entropy_and_checksum
    words: List[str] = []
    for _ in range(num_words):
        print(hex(remaining_data & 0b111_1111_1111), remaining_data & 0b111_1111_1111);
...
def decode_phrase(phrase: str) -> bytes:
    ...
    for word in words:
        ...
        try:
            ...
            wordnums.append(WORD_TO_INDEX_TABLE[word])
            print(WORD_TO_INDEX_TABLE[word], bin(WORD_TO_INDEX_TABLE[word]))
        except KeyError:
            raise DecodingError(
    ...
    print("checksum", checksum, "bits", num_bits_checksum, "computed", checksum_for_verification)
    print([(v, v & (2 ** num_bits_checksum - 1)) for v in data])
...
if __name__ == '__main__':
     print(decode_phrase(sys.stdin.read()).hex())
#    print(encode_bytes(bytearray(sys.stdin.read().encode())))
